...
let handler = async (m, { conn, command, args, text, usedPrefix, DevMode }) => {
  try {
    ...
    let _lmao = args[0]
    let Lmao = `Only support 1, 10, 100, 1000\n\nUsage example: *${usedPrefix}open ${args > 2 ? _lmao : pickRandom(['common', 'uncommon', 'mythic', 'legendary'])} 10*`
...
`.trim()
                    ...
                    } else conn.reply(m.chat, 'Your common crate is not enough', m)
                    break
                case '10':
...
`.trim()
                    if (global.DATABASE._data.users[m.sender].uncommon >= 1) {
                        ...
                        if (umc > 0) {
                            m.reply(`*Congratulations, you got the rare item*\n${umc} Mythic Crate`)
                            global.DATABASE._data.users[m.sender].mythic += umc * 1
                        }
                    } else conn.reply(m.chat, 'Your Uncommon Crate is not enough', m)
                    break
                case '10':
...
`.trim()
                    if (global.DATABASE._data.users[m.sender].mythic >= 1) {
                        ...
                        if (ml > 0) {
                            m.reply(`*Congratulations, you got the epic item*\n${ml} Legendary Crate`)
                            global.DATABASE._data.users[m.sender].legendary += ml * 1
                        }
                    } else conn.reply(m.chat, 'Your Mythic Crate is not enough', m)
                    break
                case '10':
...
`.trim()  
                    if (global.DATABASE._data.users[m.sender].legendary >= 10) {
                        ...
                        if (ll1 > 0 || lpp1 > 0) {
                             m.reply(`*Congratulations, you got the epic item*${ll1 > 0 ? `\n${ll1} Legendary Crate` : ''}${lpp1 > 0 ? `\n${lpp1} Pet Crate` : ''}`)
...
`.trim()  
                    if (global.DATABASE._data.users[m.sender].legendary >= 100) {
                        ...
                        if (lmm2 > 0) {
                            m.reply(`*Congratulations, you got a rare item, namely*\n${lmm2} Mythic Crate`)
...
`.trim()  
                    if (global.DATABASE._data.users[m.sender].legendary >= 1000) {
                        ...
                        if (ll3 > 0 || lpp3 > 0) {
                             m.reply(`*Congratulations, you got the epic item*${ll3 > 0 ? `\n${ll3} Legendary Crate` : ''}${lpp3 > 0 ? `\n${lpp3} Pet Crate` : ''}`)
        ...
        case 'pet':
            ...
            if (global.DATABASE._data.users[m.sender].pet > 0) { 
                ...
                if (_pet == 'kucing' && kucing > 0) {
                    ...
                    conn.reply(m.chat, `You already have a pet ${_pet}, Your reward is replaced with 2 potions${mknp > 0 ? ` Dan ${mknp} Makanan Pet` : ''}`, m)
