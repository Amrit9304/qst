...
let handler = async (m, { conn, usedPrefix, DevMode }) => { 
    try { 
        ...
        if (global.DATABASE._data.users[m.sender].healt > 79) {
            ...
            let str = `
♥️Your life is reduced -${healt * 1} because you have adventured to ${pickRandom(['Japan', 'India', 'Bali', 'America', 'Korea', 'Antartica', 'Arab', 'Amazon Forest', 'Mars', 'Germany', 'Finlandia', '💭To the world of dreams', '🔚End of the world', 'Mars', '🌑Moon', 'Pluto', '☀️Sun', '❣️Her heart', '...'])} and get
*🎆Exp:* ${exp} 
*💰Money:* ${uang}
*🪵Wood:* ${kayu}
*🕸️String:* ${string}
...
`.trim()
        ...
        } else conn.sendButton(m.chat, 'Minimum 80 health♥️ required for adventure,so buy lives first by typing*' + usedPrefix + 'shop buy potion <amount>*\nand type *' + usedPrefix + 'use potion <amount>*', `Quantum Bot`, `Heal`, `.use potion all`, m)
    } catch (e) {
        console.log(e)
...
handler.help = ['adventure', 'work']
handler.tags = ['rpg']
handler.command = /^(adventure|work)$/i
